...
- [x] **METR-01**: Script checks swingup success: q2 crosses ±π AND holds within ±2° of ±π for at least 1 continuous second
- [x] **METR-02**: Script checks participation: |q2| exceeds π/2 at any point during the attempt
- [x] **METR-03**: Script computes swingup time: from aligned start (first non-zero command) to first ±π crossing on the hardware signal
- [x] **METR-04**: Script computes SMAPE between hardware q2 and simulation q2 using `interp1` for resampling (not timeseries arithmetic)
- [ ] **METR-05**: SMAPE window is configurable: fixed time (e.g., 5s), angle threshold (e.g., until |q2| > π/2), or until swingup completion *(partial — D-09 hybrid policy applied; awaiting instructor confirmation)*
- [x] **METR-06**: SMAPE computation guards against division-by-zero when both signals are near zero (denominator check)
- [x] **METR-07**: SMAPE computation handles angle wrapping near ±π using angular difference formula (`mod(hw-sim+pi, 2*pi)-pi`)

### Scoring

- [x] **SCOR-01**: Script uses best-per-metric scoring: fastest swingup time from any successful attempt, lowest SMAPE from any attempt where participation achieved (|q2| > π/2)
- [x] **SCOR-02**: Script ranks 4 stepper teams on swingup time: 1st=2pt, 2nd=1pt, 3rd=0.5pt, 4th=0pt
- [x] **SCOR-03**: Script ranks 4 stepper teams on SMAPE: 1st=2pt, 2nd=1pt, 3rd-4th=0.5pt
- [x] **SCOR-04**: Script scores BLDC team on absolute SMAPE: 0-40%=4pt, 40-80%=3pt, 80-120%=2pt, 120-160%=1pt, 160-200%=0pt
- [x] **SCOR-05**: Script awards 1 participation point to any team where |q2| > π/2
- [x] **SCOR-06**: Script computes total points per team (max 5) and produces a ranked leaderboard

### Output

- [x] **OUTP-01**: Script produces a MATLAB table with columns: Team, BestSwingupTime, BestSMAPE, TimePoints, SMAPEPoints, ParticipationPoint, TotalPoints, Rank
- [x] **OUTP-02**: Script exports leaderboard to CSV and Excel (`.xlsx`) at finalization (when scorer issues the finalize command)
...
- [x] **OUTP-04**: Script prints per-team diagnostic summary: N attempts loaded, best time, best SMAPE, participation status
- [x] **OUTP-05**: All tunable parameters (thresholds, SMAPE window mode, angle tolerances) are in a `cfg` struct at the top of the script
...
| METR-05 | Phase 3 | Partial (D-09) |
| METR-06 | Phase 3 | Complete |
| METR-07 | Phase 3 | Complete |
...
- Satisfied: 28 (16 updated from audit)
- Partial: 1 (METR-05 — D-09 override, awaiting instructor confirmation)
- Mapped to phases: 29
- Unmapped: 0

---
*Requirements defined: 2026-04-02*
*Last updated: 2026-04-06 — checkboxes and traceability updated from v1.0 milestone audit*
